...
- The Cover Flow stage carries a static, single-colour wood-grain field. Warm ink SVG hairlines flow around a restrained knot and compress or stretch so local line density—not transparency or a gradient—sets the light and dark grain. Each cover extends a clipped mirrored reflection below its lower edge; the same wood-grain field textures and dissolves that reflection instead of adding a separate translucent surface.
