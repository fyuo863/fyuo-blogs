...
- The halftone canvas fills the warm-paper home cover. The title bounds use a compact default dot radius (0.39 cell); the surrounding cover uses 0.78 cell. A damped two-dimensional wave-heightfield injects stronger sources at randomized intervals along the left edge, and pointer presses inject a local ripple; stronger wave peaks shrink the fixed-centre dots to reveal more cobalt.
