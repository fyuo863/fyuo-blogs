...
- The Cover Flow stage carries a static, warped solid-colour moire field. Cobalt and vermilion SVG hairlines compress and stretch against their own offsets, so their local density—not transparency or a gradient—sets the light and dark bands. Each cover extends a clipped mirrored reflection below its lower edge; the same warped field colours and dissolves that reflection instead of adding a separate translucent surface.
