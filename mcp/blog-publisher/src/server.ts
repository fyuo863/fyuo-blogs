...
export function createBlogPublisherServer(
  ...
  const effectiveConfig: BlogPublisherConfig = options?.apiKey
  ...
  server.tool(
    "blog_login",
    "Verify that the current blog API key can reach the blog backend and return the active identity. Use this for connectivity or authentication checks; it does not prompt for a username or password.",
    {},
    async () => {
  ...
  server.tool(
    "blog_create_post",
    "Create a new blog post in the backend and return the created article record.",
    {
  ...
  server.tool(
    "blog_update_post",
    "Update an existing blog post by article ID and return the updated article record.",
    {
  ...
  server.tool(
    "blog_upload_image",
    "Upload an image to the blog backend and return the hosted URL plus a ready-to-paste markdown snippet. Use this before blog_create_post when a post should contain images.",
    {
      filename: z
        .string()
        .min(1)
  ...
  server.tool(
    "blog_list_posts",
    "List blog posts with pagination and return the backend paging result including data and totals.",
    {
  ...
  server.tool(
    "blog_get_post",
    "Fetch a single blog post by article ID and return the full article record.",
    {
  ...
  server.tool(
    "blog_search_posts",
    "Search blog posts by keyword and return matching articles with the backend's total count.",
    {
